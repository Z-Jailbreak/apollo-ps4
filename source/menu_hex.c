...
void Draw_HexEditor_Ani(const hexedit_data_t* hex)
{
    for (int ani = 0; ani < MENU_ANI_MAX; ani++)
    {
        ...
        DrawHeader_Ani(cat_opt_png_index, "Editor Hexadecimal", strrchr(hex->filepath, '/')+1, APP_FONT_TITLE_COLOR, 0xffffffff, ani, 12);
...
{
    DrawHeader(cat_opt_png_index, 0, "Editor Hexadecimal", strrchr(hex->filepath, '/')+1, APP_FONT_TITLE_COLOR | 0xFF, 0xffffffff, 0);
